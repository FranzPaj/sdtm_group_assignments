...
def generate_orbit_inputs(setup_file, m, orbit_regime='LEO'):
    ...
    if orbit_regime == 'GEO':
        params['Xo_true'] = np.reshape(
            [3.57133892e7, 2.23987784e7, -6.69976699e4,
             -1.63445654e3, 2.60499214e3, 6.88190044], (6, 1))

    if m == 3:
        params['Rk'] = np.diag(
            [1., (360 * arcsec2rad) ** 2., (360 * arcsec2rad) ** 2.])
    elif m == 2:
    ...
    params['sensor_ecef'] = np.reshape([-5466071., -2403990., 2242473.],
                                       (3, 1))
...
def generate_orbit_truth(setup_file, truth_file, intfcn):
    ...
    tin = (tvec[0], tvec[-1])
    t_truth = tvec
    output = solve_ivp(intfcn, tin, Xo.flatten(), method=method,
                       args=(params,), rtol=rtol, atol=atol, t_eval=tvec)
...
def run_estimator_orbit_model(setup_file, truth_file, meas_file, output_file,
                              intfcn, meas_fcn, estimator, Qeci=None,
                              Qric=None):
    ...
    # Orbit Model Params
    params['step'] = 60.
    pert_vect = np.multiply(np.sqrt(np.diag(params['Po'])),
                            np.random.randn(6, ))
    ...
    if estimator == 'batch':
        t_output = t_truth
        t_output, Xref_mat, P_mat, resids = Estimators.ls_batch(Xo_ref, Po,
                                                                t_obs,
                                                                t_output,
                                                                obs_data,
                                                                intfcn,
    ...
    # Execute UKF
    if estimator == 'ukf':
        t_output, Xref_mat, P_mat, resids = Estimators.ukf(Xo_ref, Po, t_obs,
                                                           obs_data, intfcn,
                                                           meas_fcn, params)
...
if __name__ == '__main__':
    ...
    generate_orbit_inputs(setup_file, 3, 'GEO')

...
def propagate_orbit(Xo, tvec, state_params, int_params, bodies=None, latlon_flag=False):
    '''
    ...
    dependent_variables_to_save = [
        propagation_setup.dependent_variable.latitude("0", "Earth"),
        propagation_setup.dependent_variable.longitude("0", "Earth"),
        propagation_setup.dependent_variable.received_irradiance_shadow_function("0", "Sun")
    ]
    ...
    if latlon_flag:
        dep_var = result2array(dynamics_simulator.dependent_variable_history)
        latitudes = dep_var[:, 1]
        longitudes = dep_var[:, 2]
        shadow = dep_var[:, 3]
        return tout, Xout, latitudes, longitudes, shadow

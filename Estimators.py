...
def ls_batch(Xo_ref, Po_bar, t_obs, t_output, obs_data, intfcn, meas_fcn,
             params):
    '''
    ...
    while not conv_flag:
        ...
        for kk in range(L):
            ...
            if tk_prior == tk:
                ...
                output = solve_ivp(intfcn, tin, int0, method='DOP853',
                                   args=(params,), rtol=rtol, atol=atol)
                intout = output['y'].T
...
def ukf(Xo, Po, t_obs, obs_data, intfcn, meas_fcn, params):
    ...
    for kk in range(L):
        ...
        Xrep = np.tile(Xk, (1, n))
        chi = np.concatenate((Xk, Xrep + (gam * sqP), Xrep - (gam * sqP)),
                             axis=1)
        ...
        chi_bar = np.concatenate(
            (Xbar, Xrep + (gam * sqP), Xrep - (gam * sqP)), axis=1)
        ...
        sqP = np.linalg.cholesky(Pk)
        Xrep = np.tile(Xk, (1, n))
        chi_k = np.concatenate((Xk, Xrep + (gam * sqP), Xrep - (gam * sqP)),
                               axis=1)
...
def int_twobody_j2(t, X, params):
    ...
    dX[3] = -GM * x / r ** 3. - 1.5 * J2 * R ** 2. * GM * (
                (x / r ** 5.) - (5. * x * z ** 2. / r ** 7.))
    dX[4] = -GM * y / r ** 3. - 1.5 * J2 * R ** 2. * GM * (
                (y / r ** 5.) - (5. * y * z ** 2. / r ** 7.))
    dX[5] = -GM * z / r ** 3. - 1.5 * J2 * R ** 2. * GM * (
                (3. * z / r ** 5.) - (5. * z ** 3. / r ** 7.))
